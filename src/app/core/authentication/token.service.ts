...
export class TokenService {
   ...
   */
  checkInsecureToken() {
    let origin = localStorage.getItem('ch-auth-insecure-origin');
    let tokenString = localStorage.getItem('ch-auth-insecure-token');
      ...
      return this.configService.getServices()
        ...
        .do(urls => {
          if (urls.indexOf(origin) !== -1) {
            ...
            let roles = JSON.parse(token.rolesJson);
