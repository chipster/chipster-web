...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.configService.get(ConfigService.KEY_HOME_PATH).subscribe(path => {
      if (path) {
      ...
      this.errorService.headerError('failed to get the path of the home page', true);
    });

    this.configService.get(ConfigService.KEY_HOME_HEADER_PATH).subscribe(path => {
      if (path) {
        this.homeHeaderFile = this.routeService.basename(path);
        this.homeHeaderPath = this.routeService.dirname(path) + '/';
        console.log('loading custom home page header', this.homePath, this.homeFile);
      }
    }, err => {
