...
export class SessionResource {
  ...
  deleteRule(sessionId: string, ruleId: string) {
    ...
    }).map(() => new SessionCopyMapping().set(createdSessionId, datasetIdMap, jobIdMap));
	}
}

export class SessionCopyMapping {
  sessionId: string;
  datasetIdMap: Map<string, string>;
  jobIdMap: Map<string, string>;

  set(sessionId, datasetIdMap, jobIdMap) {
  ...
  dummy(sessionData: SessionData) {
    ...
    for (let id in sessionData.datasetsMap.keys()) {
      this.datasetIdMap.set(id, id);
    }

    for (let id in sessionData.jobsMap.keys()) {
      this.jobIdMap.set(id, id);
    }
