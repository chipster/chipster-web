...
export class NativeElementService {
  ...
  disableGestures(nativeElement) {
    ...
    nativeElement.addEventListener("mousewheel", event => {
      ...
      if (
        nativeElement.scrollLeft + event.deltaX < 0 ||
        nativeElement.scrollLeft + event.deltaX > maxX
      ) {
        event.preventDefault();

        // Manually set the scroll to the boundary
        nativeElement.scrollLeft = Math.max(
          0,
          Math.min(maxX, nativeElement.scrollLeft + event.deltaX)
        );
      }
    });
  }
