...
describe('TSVFile', function () {
    ...
    var tsv = createTSVFile();
    describe('construction', function () {
        it('should have equal sized header and bodyrows', function () {
            expect(tsv.headers.size()).toBe(tsv.body.rows[0].size());
        });
    });
    describe('getColumnIndex', function () {
        it('should return correct header index', function () {
    ...
    describe('getColumnDataByHeaderKey', function () {
        it('should return identifier-column', function () {
            var column = tsv.getColumnDataByHeaderKey('identifier');
            expect(column[0]).toBe('7369_at');
            expect(column[1]).toBe('10610_at');
            expect(column[2]).toBe('22981_at');
        });
        it('should return symbol-column', function () {
            var column = tsv.getColumnDataByHeaderKey('symbol');
            expect(column[0]).toBe('UMOD');
            expect(column[1]).toBe('ST6GALNAC2');
            expect(column[2]).toBe('NINL');
        });
        it('should return chip.microarray006.cel-column', function () {
            var column = tsv.getColumnDataByHeaderKey('chip.microarray006.cel');
            expect(column[0]).toBe('4.97');
            expect(column[1]).toBe('6.69');
            expect(column[2]).toBe('6.12');
        });
    });
    describe('getColumnDataByHeaderKeys', function () {
        var symbolColumn = tsv.getColumnDataByHeaderKey('symbol');
        var chipcolumn6 = tsv.getColumnDataByHeaderKey('chip.microarray006.cel');
        var columnsByHeaderKeys = tsv.getColumnDataByHeaderKeys(['symbol', 'chip.microarray006.cel']);
        it('should return arrays containing items by columnkeys', function () {
